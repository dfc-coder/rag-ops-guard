...
from rag_ops_guard.configstore.runtime import RuntimeConfigResolver, snapshot_for_values
...
class FakeStore:
    ...
    def get_head(self) -> ConfigHead | None:
        ...
        return ConfigHead(revision_no=3, content_hash="db-head-hash", published_at=self.published_at)
...
def test_db_value_wins_over_environment_and_hash_is_effective() -> None:
    now = [100.0]
    store = FakeStore({"retrieval_top_k": 9, "retrieval_context_k": 3}, published_at=88.0)
    resolver = RuntimeConfigResolver(
        store=store,
        bootstrap=Settings(retrieval_top_k=2, retrieval_context_k=2),
        source="db",
        clock=lambda: now[0],
...
def test_l1_cache_is_revision_keyed_and_head_is_checked_only_after_ttl() -> None:
    ...
    resolver = RuntimeConfigResolver(
        store=store, bootstrap=Settings(), source="db", head_ttl_s=45.0, clock=lambda: now[0]
    )
...
def test_dynamodb_outage_serves_stale_tuning_then_fails_closed_after_max_stale() -> None:
    now = [0.0]
    store = FakeStore(
        {"retrieval_top_k": 7, "retrieval_domain_min_relevance": 0.6, "retrieval_min_relevance": 0.7},
        published_at=0.0,
    )
    resolver = RuntimeConfigResolver(
        store=store,
...
def test_fallback_order_is_s3_then_baked_then_code_default() -> None:
    ...
    s3_snapshot = snapshot_for_values(bootstrap, {"retrieval_top_k": 11}, revision_no=2, created_at=10.0)
    baked_snapshot = snapshot_for_values(bootstrap, {"retrieval_top_k": 12}, revision_no=1, created_at=20.0)
    resolver = RuntimeConfigResolver(
        store=store, bootstrap=bootstrap, source="db", clock=lambda: now[0], wall_clock=lambda: now[0],
        s3_loader=lambda: s3_snapshot, baked_loader=lambda: baked_snapshot,
    )
    s3 = resolver.resolve()
    ...
    resolver = RuntimeConfigResolver(
        store=store, bootstrap=bootstrap, source="db", clock=lambda: now[0], wall_clock=lambda: now[0],
        s3_loader=lambda: None, baked_loader=lambda: baked_snapshot,
    )
    ...
    resolver = RuntimeConfigResolver(
        store=store, bootstrap=bootstrap, source="db", clock=lambda: now[0], wall_clock=lambda: now[0],
        s3_loader=lambda: None, baked_loader=lambda: None,
    )
