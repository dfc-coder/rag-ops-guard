...
DOCKER_PROXY_CONTAINER_NAME ?= rag-ops-docker-proxy
...
local-up:
	mkdir -p "$(MODEL_DIR)"
	uv run python scripts/local/podman_api_bridge.py start --socket "$(PODMAN_API_SOCKET)"
	$(COMPOSE) up -d
...
local-core-up:
	...
	$(COMPOSE) up -d floci docker-proxy llama-gen
	uv run python scripts/wait_core.py

local-down:
	-uv run python scripts/openvino_runtime.py down
	-$(COMPOSE) down
	-uv run python scripts/local/podman_api_bridge.py stop --socket "$(PODMAN_API_SOCKET)"
