...
def _timings(payload: dict[str, Any]) -> tuple[int, int]:
    ...
    if not isinstance(timings, dict):
        raise SystemExit("llama.cpp timings missing")
...
def main() -> None:
    ...
    common = (
        "RAG Ops Guard cache validation. "
    ...
    first = _request(base_url, prefix, "Return alpha.")
    second = _request(base_url, prefix, "Return beta.")
    ...
    if not passed:
        raise SystemExit("KV cache validation failed")
